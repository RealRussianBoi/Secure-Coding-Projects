...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        {
            return std::numeric_limits<T>::max(); // sentinel: overflow
        }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        if (result < decrement)
        {
            return std::numeric_limits<T>::min(); // sentinel: underflow
        }
...
void test_overflow()
{
    ...
    // Print the inputs being used for the safe addition test (expected to succeed)
    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";

    // Call the add_numbers function with safe values
    T result = add_numbers<T>(start, increment, steps);

    // Check if the returned value equals the sentinel (std::numeric_limits<T>::max())
    // If it does, that indicates overflow occurred during the loop
    if (result == std::numeric_limits<T>::max())
        std::cout << "Overflow occurred!" << std::endl;
    else
        // Otherwise, print the valid result
        std::cout << +result << std::endl;

    // Now test the same function but intentionally cause overflow (steps + 1)
    std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";

    // Call the function with more iterations to trigger overflow
    result = add_numbers<T>(start, increment, steps + 1);

    // Again, check if the sentinel value is returned
    if (result == std::numeric_limits<T>::max())
        std::cout << "Overflow occurred!" << std::endl;
    else
        std::cout << +result << std::endl;
...
void test_underflow()
{
    ...
    // Print the inputs being used for the safe subtraction test (expected to succeed)
    std::cout << "\tSubtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";

    // Call the subtract_numbers function with values that should not cause underflow
    T result = subtract_numbers<T>(start, decrement, steps);

    // Check if the returned value equals the sentinel (std::numeric_limits<T>::min())
    // If so, underflow occurred during one of the steps
    ...
    // Now test the same function but intentionally cause underflow (steps + 1)
    std::cout << "\tSubtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";

    // Call the function with more iterations to trigger underflow
    result = subtract_numbers<T>(start, decrement, steps + 1);

    // Again, check for underflow by comparing to the sentinel value
    if (result == std::numeric_limits<T>::min())
        std::cout << "Underflow occurred!" << std::endl;
    else
        std::cout << +result << std::endl;
